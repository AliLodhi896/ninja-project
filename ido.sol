...
contract NinjaStarter is ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public offeringAmount = 2500000 * 1e18; // 2.5 million

    // IDO starting date
    uint256 public startDate = 1640581200; 

    // limit on each address on buy
    uint256 public buyCap = 0;

    // offering token price in BUSD
    uint256 public buyPrice = 50000000000000000; // 0.05 BUSD
    ...
    bool public isNinjaHolderReward = true;

    // number of ninja token required to receive reward
    uint256 public NinjaRequiredforReward = 100;

    // ninja holder reward in percentage
    uint256 ninjaHolderReward = 5;
    ...
    //locked liquidity %
    uint256 public liquidityInPercentage = 30;
    ...
    modifier checkBuyCondition(uint256 amount ,address _user) {
        ...
        require(
            offeringToken.balanceOf(address(this)) > 0,
            "Insufficient buy amount"
        );
        _;
    }

  function getRefund() external refundIsOpen notClaimedOrRefunded nonReentrant {
    require(refundOpen, "IDO is not open for refund");
    require(!Refundclaimed[msg.sender], "Already claimed or refunded");
    ...
    function _checkout(address beneficiary, uint256 _amount) internal {
        ...
        if(isNinjaHolderReward && isWhitelisted(beneficiary)){
            uint256 reward = _amount.mul(ninjaHolderReward).div(100);
    ...
    function setWhitelistSettings(bool _isNinjaHolderReward , uint256 _NinjaRequiredforReward) public onlyOwner {
        isNinjaHolderReward = _isNinjaHolderReward;
    ...
    function isWhitelisted(address _address) public view returns (bool) {
        ...
        return _deposited >= (NinjaRequiredforReward *(10**18)) ? true : false;
    }
