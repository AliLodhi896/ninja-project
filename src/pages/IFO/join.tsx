...
export default function IfoJoin({
  match: {
    params: { symbol },
  },
  ...
  const loadEstimatedToken = async (fieldTyping: 'input' | 'output', value: any) => {
    ...
    try {
      if (fieldTyping === 'input') {
        ...
        const contract = getNinjaStarterContract(
        ...
        if (inputCurrency?.symbol === 'BNB') {
          const estimatedAmount = await contract.getEstimatedTokensBuyWithBNB(
  ...
  const loadPoolDetail = async () => {
    ...
    if (data && data?.poolInfo?.status === 'active') {
      ...
      history.push(`/ninja-starter/${symbol}`);
    }
    if (!account) {
  ...
  const onBuy = useCallback(async () => {
    ...
    try {
      ...
      if (inputCurrency?.symbol === 'BNB') {
        ...
        const contract = getNinjaStarterContract(poolAddress, chainId, library, account);
          ...
          <div className="col-lg-11 m-0">
            {!poolInfo ? (
              ...
              <>
                <BackButtonWrapper>
                  <Button
                    onClick={() => history.push(`/ninja-starter/${symbol}`)}
                    className="ninja-button"
                    style={{ border: '0px solid rgb(240, 185, 11)', backgroundColor: '#12161c' }}
                  >
                    <FontAwesomeIcon icon={faArrowLeft} size="1x" /> Back to Pool
                  </Button>
                </BackButtonWrapper>
                <div style={{ textAlign: 'center', padding: '25px 0 25px 0' }}>
                  <img
                    ...
                    alt=""
                  />
                  <h1 className="subTitle pt-2">{poolInfo?.name}</h1>
                </div>
                <div>
